...
def evaluate(env_id, maddpg, n_episode=1, episode_length=25):
    # 对学习的策略进行评估,此时不会进行探索
    env = make_env(env_id)
...
for i_episode in range(num_episodes):
    ...
    if (i_episode + 1) % 100 == 0:
        ...
        for agent_i in range(len(env.agents)):
            agent = env.agents[agent_i]
            flag = os.path.exists(agent_path)
            if not flag:
                os.makedirs(agent_path)
            torch.save(maddpg.agents[agent_i].actor.state_dict(),f"models"+"\\"+scenario+"\\"+f"agent_{agent_i}_actor_{scenario}_{timestamp}.pth")
...
for i, agent_name in enumerate(["adversary_0", "agent_0", "agent_1"]):
    ...
    plt.plot(
        ...
        rl_utils.moving_average(return_array[:, i], 1))
    plt.xlabel("Episodes")
    plt.ylabel("Returns")
